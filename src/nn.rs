...
type Callback = Box<dyn Fn(u8, u32, usize, usize, f64)>;
...
impl<'a, 'b> Trainer<'a, 'b> {
    ...
    /// get trained!
    pub fn go(&mut self, callback: Callback) -> f64 {
...
impl NN {
    ...
    fn train_details(
        ...
        self.train_incremental(examples, rate, momentum, halt_condition, callback)
    }
    ...
    fn train_incremental(
        ...
        loop {
            ...
            let mut i: usize = 0;
            callback(1, epochs, i, examples.len(), training_error_rate);
            for &(ref inputs, ref targets) in examples.iter() {
                ...
                i += 1;
                callback(2, epochs, i, examples.len(), training_error_rate);
            }

            callback(3, epochs, i, examples.len(), training_error_rate);

            epochs += 1;
        }

        callback(4, epochs, 0, examples.len(), training_error_rate);
